  ...
  <div className="flex flex-col gap-6 sm:gap-8 items-center p-6 sm:p-8 bg-white dark:bg-stone-900 text-stone-700 dark:text-stone-100 rounded-xl my-6 sm:my-8 border border-stone-100 dark:border-stone-800 shadow-lg transition-colors">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 w-full">
      {STACK.map((group, index) => (
        <motion.div
          key={group.title}
          className="rounded-2xl border border-stone-100 dark:border-stone-800 bg-stone-50 dark:bg-stone-950/40 p-5 sm:p-6"
          ...
          <div className="flex flex-wrap gap-2">
            {group.items.map((item) => (
              <span
                ...
                className="px-3 py-1 text-[11px] rounded-full bg-white dark:bg-stone-900 border border-stone-100 dark:border-stone-800 text-stone-600 dark:text-stone-300"
              >
                {item}
              </span>
            ))}
